  ...
  const tinhHocPhi = async (hocKyId: string) => {
    ...
    try {
      ...
      if (result.isSuccess && result.data) {
        const { successCount, totalProcessed, failedCount, errors } =
          result.data;

        // ✅ Show success notification | ${successCount}/${totalProcessed}
        openNotify({
          message: `Tính học phí thành công cho sinh viên`,
          type: "success",
        });
        ...
        if (failedCount > 0 && errors.length > 0) {
          ...
          openNotify({
            message: `Có ${failedCount} sinh viên lỗi:\n${errorMessages}${
              errors.length > 5 ? "\n..." : ""
            }`,
            type: "warning",
          });
